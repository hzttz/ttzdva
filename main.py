...
class VoiceRecorderCLI:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                headers = {
                    ...
                    "Authorization": f"Bearer {self.STEPFUN_API}"
                }
    ...
    async def play_audio(self, file_path):
        ...
        try:
            ...
            # 停止后清理
            sd.stop()
            os.remove(file_path)  # 播放完成后删除临时文件
            
        except Exception as e:
            print(f"播放失败: {str(e)}")
            await self.save_error_log("play_error", str(e))
        finally:
            with self.play_lock:
                self.is_playing = False
                if self.play_queue:
    ...
    async def llm_format(self, prompt_u):
        ...
        payload = {
            ...
            "messages": [
                {"role": "system", "content": '''
用户输入内容可能包含一些可转化为python执行的命令，为了辅助理解，用户会添加一些背景信息：比如数据库配置信息，邮件服务器配置信息，在不涉及到背景信息操作时，输出内容不要强行与背景信息关联，然后你需要识别这些指令，并转化为可直接执行的python代码,如果一个命令包含多个子命令，其中包含不可执行部分（缺乏参数），对该子命令代码做注释，确保最大化执行可执行部分；
对用户内容中的非命令部分，使用口语化，情绪饱满，积极阳光，能够给用户提供情绪价值的高情商回答；
最后总结用户的输入，总结以上的处理结果，需要使用更贴近日常交流的语言，严格按照以下格式输出。注意：command 可以为空，resp_to_user 非空。格式输出示例：" 
{
